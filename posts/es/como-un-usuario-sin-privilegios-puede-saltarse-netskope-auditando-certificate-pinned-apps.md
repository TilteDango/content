...
---
# Cómo un usuario sin privilegios puede saltarse Netskope: auditando Certificate Pinned Apps
...
Todo esto es en un entorno idílico para la aplicación, pero la realidad es otra. En la práctica, esto no siempre funciona. Hay aplicaciones que, sencillamente, se niegan a operar si su tráfico pasa por un intermediario. 
...
> Un matiz importante es que para las `Certificate Pinned Apps` definidas en Netskope se puede implementar con `Tunnel Mode` lo que hace que salga por la nube de Netskope (así evitando abrir destinos en el firewall perimetral) pero sin ningún tipo de inspección. 
...
> También se pueden sacar si se tiene acceso a interfaz gráfica desde el icono de Netskope -> Save Logs
...
Obviamente estamos hablando de una prueba de concepto simple, acceder a una web que no se puede, legítima, pero si pensamos más allá podemos ver nuevos vectores de ataque que nos facilita esta mala configuración.

----
## La respuesta de fabricante ante este fallo

Tras documentar el hallazgo, se reportó a Netskope siguiendo el canal habitual del fabricante. Es una mala configuración pero se podía enumerar esta configuración sin ningún tipo de restricción. La respuesta de fabricante se puede resumir en: es el comportamiento esperado del producto.

La argumentación del fabricante tiene tres patas válidas que hacen que sea responsabilidad de los administradores o cliente y no del producto.

La primera es que las `Certificate Pinned Apps` deben existir para resolver un problema real y no puede ser ignorado, y que cualquier mecanismo que permita exceptuar tráfico del `steering` va a tener, por definición, una superficie de uso indebido. No es un fallo de diseño, es una mala configuración.
